...
def save_graph(lat, lon, zoom, output_dir="static", model_name="modele3"):
    ...
    fig, ax = plt.subplots()
    ax.plot(t, y)
    ax.set_title(f"modele3 - Température - Zoom {{zoom}}")
    ax.set_xlabel("Temps (jours)" if zoom != "journalier" else "Heure")
    ax.set_ylabel("Température (°C)")
    fig.savefig(os.path.join(output_dir, f"modele3_graph_{{zoom}}.png"))
    plt.close(fig)
